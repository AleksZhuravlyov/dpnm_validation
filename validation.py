...
grid_block_y_n = 25  # grid_block_n is a number of diffusion grid blocks
...
params = {'time': float(time),
          ...
          'len_x': float(len_x),
          'len_z': float(len_z),
          'grid_block_y_n': int(grid_block_y_n),
...
points_dims = np.array([2, params['grid_block_y_n'] + 1, 2], dtype=np.int32)
...
grid_block_x_n = 100  # grid_block_n is a number of convection grid blocks
...
with FoamFile(file_name) as f:
    foam_content = f.read()
foam_content['endTime'] = params['time']
